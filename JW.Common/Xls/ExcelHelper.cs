...
namespace JW.Common.Xls
{
    ...
    public class ExcelHelper
    {
        ...
        ///// <param name="excelConfigList">导出设置包含文件名、标题、列设置</param>
        //public ActionResult ExcelDownload(DataSet dsSource, List<ExcelGridModel[]> columnList, string fileName)
        //{
        //    var excelConfig = ConvertExcelGridModelToConfig(columnList, fileName);
        //    var stream = ExportMemoryStream(dsSource, excelConfig);
        //    return new FileStreamResult(stream, MIMEType.xls) { FileDownloadName = JointXls(fileName) };
        //}
        ...
        ///// <param name="dsSource">DataSet数据源</param>
        ///// <param name="excelConfig">导出设置包含文件名、标题、列设置</param>
        //public ActionResult ExcelDownload(DataSet dsSource, List<ExcelConfig> excelConfig, string fileName)
        //{
        //    var stream = ExportMemoryStream(dsSource, excelConfig);
        //    return new FileStreamResult(stream, MIMEType.xls) { FileDownloadName = JointXls(fileName) };
        //}
        ...
        /// <summary>
        /// DataSet导出到Excel文件 Export()
        /// </summary>
        /// <param name="dsSource">DataTable数据源</param>
        /// <param name="excelConfig">导出设置包含文件名、标题、列设置</param>
        public string ExcelExportToFile(DataSet dsSource, List<ExcelConfig> excelConfig, string fileName)
        {
            fileName = JointXls(fileName);
            using (MemoryStream ms = ExportMemoryStream(dsSource, excelConfig))
            {
                using (FileStream fs = new FileStream(fileName, FileMode.Create, FileAccess.Write))
                {
                    byte[] data = ms.ToArray();
        ...
        /// <summary>
        /// DataSet导出到Excel的MemoryStream Export()
        /// </summary>
        /// <param name="dsSource">DataSet数据源</param>
        /// <param name="excelConfig">导出设置包含文件名、标题、列设置</param>
        public MemoryStream ExportMemoryStream(DataSet dsSource, List<ExcelConfig> excelConfigList)
        {
            ...
            if (dsSource.ExDataSetNotNullEmpty())
            {
                for (int s = 0; s < dsSource.Tables.Count; s++)
                {
                    DataTable dtSource = dsSource.Tables[s];
                    var columnEntity = new List<ColumnModel>();
                    var excelConfig = new ExcelConfig();
                    if (excelConfigList.Count > s)
                    {
                        columnEntity = excelConfigList[s].ColumnEntity;
                        excelConfig = excelConfigList[s];
                    }
                    if (columnEntity == null || columnEntity.Count == 0)
                    {
                        if (columnEntity == null)
                    ...
                    else
                    {
                        ...
                        {
                            DataColumn column = dtSource.Columns[i];
                            if (columnEntity[colint].Column != column.ColumnName)
                            {
                                dtSource.Columns.Remove(column.ColumnName);
                            }
                            else
                            {
                                i++;
                                if (colint < columnEntity.Count - 1)
                                {
                                    colint++;
                                }
                            }
                    ...
                    ISheet sheet = workbook.CreateSheet(dtSource.TableName.ExStrNotNull() ? dtSource.TableName : s.ToString());
        ...
        #endregion DataSet导出到Excel的MemoryStream
        ...
        private List<ExcelConfig> ConvertExcelGridModelToConfig(List<ExcelGridModel[]> columnList, string fileName)
        {
            ...
            foreach (var item in columnList)
            {
                ...
                excelconfig.IsAllSizeColumn = true;
                excelconfig.ColumnEntity = new List<ColumnModel>();
                foreach (ExcelGridModel columnModel in item)
                {
                    excelconfig.ColumnEntity.Add(new ColumnModel()
                    {
                        Column = columnModel.name,
