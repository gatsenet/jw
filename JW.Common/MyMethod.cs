...
namespace JW.Common
{
    public class MyMethod
    {
            ...
            try
            {
                ...
                if (dic.Count > 0)
                {
                    ...
                    foreach (var d in dic)
                    {
                        if (i > 0)
                            builder.Append("&");
                        builder.AppendFormat("{0}={1}", d.Key, d.Value);
                        i++;
                    }
                }
                HttpHelper http = new HttpHelper();
                HttpItem item = new HttpItem()
                {
                    URL = builder.ToString(),//URL     必需项
                    Method = "Get",//URL     可选项 默认为Get
                    Timeout = timeout * 60 * 1000,//连接超时时间     可选项默认为100000
                    ReadWriteTimeout = 30000,//写入Post数据超时时间     可选项默认为30000
                    IsToLower = false,//得到的HTML代码是否转成小写     可选项默认转小写
                    Cookie = "",//字符串Cookie     可选项
                    UserAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:18.0) Gecko/20100101 Firefox/18.0",//用户的浏览器类型，版本，操作系统     可选项有默认值
                    //Accept = "text/html, application/xhtml+xml, */*",//    可选项有默认值
                    ContentType = isjson ? "application/json" : "text/html",//返回类型    可选项有默认值
                    IsGzip = isgzip,
                    //Referer = ApiUrlTextEdit.Text.Trim(),//来源URL     可选项
                    //Allowautoredirect = true,//是否根据３０１跳转     可选项
                    //CerPath = "d:\123.cer",//证书绝对路径     可选项不需要证书时可以不写这个参数
        ...
        public static void JsonDateStrToDate()
        {
